...
const {ARRAY_SPLIT_DELIMITER} = require("../../../../lib/commands/smapi/cli-customization-processor");
const Messenger = require("../../../../lib/view/messenger");
const jsonView = require("../../../../lib/view/json-view");
const BeforeSendProcessor = require("../../../../lib/commands/smapi/before-send-processor");
...
const {smapiCommandHandler, parseSmapiResponse} = require("../../../../lib/commands/smapi/smapi-command-handler");
...
describe("Smapi test - smapiCommandHandler function", () => {
  ...
  beforeEach(() => {
    ...
    clientStub[sdkFunctionName].toString = () =>
      "function (someJson, skillId, " + "someNonPopulatedProperty, someArray, simulationsApiRequest) { return 0};";
  ...
  it("| should display debug message when debug flag is passed", async () => {
    ...
    const messengerStub = sinon.stub(Messenger.getInstance(), "displayMessage");
    ...
    expect(messengerStub.args[2]).eql([
      "INFO",
      `${jsonView.toString({someJson: null, skillId, someNonPopulatedProperty: null, someArray: null, simulationsApiRequest: {}})}\n`,
    ]);
  ...
  it("| should display body, headers and status code when full response flag is passed", async () => {
    ...
    const result = await smapiCommandHandler(
      apiOperationName,
      flatParamsMap,
...
describe("Smapi test - parseSmapiResponse function", () => {
  ...
  it("| should show warning with status hint command when able to find one", () => {
    ...
    expect(warnStub.firstCall.lastArg).eql(
      "This is an asynchronous operation. Check the progress " +
