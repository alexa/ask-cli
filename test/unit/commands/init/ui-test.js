...
describe("Commands init - UI test", () => {
  ...
  describe("# validate ui.getSkillMetaSrc", () => {
    ...
    it("| check the validate logic from inquirer and path is empty", (done) => {
      ...
      ui.getSkillMetaSrc(() => {
        // verify
        expect(inquirer.prompt.args[0][0][0].validate("    ")).equal("Path for skill package cannot be empty.");
        done();
      });
    });
    ...
    it("| check the validate logic from inquirer and file does not exist", (done) => {
      ...
      ui.getSkillMetaSrc(() => {
        // verify
        expect(inquirer.prompt.args[0][0][0].validate("path")).equal("File path does not exist.");
        done();
      });
    });
  });
