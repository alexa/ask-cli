...
const askProfileSetupHelper = require("../../../../lib/commands/configure/ask-profile-setup-helper");
const awsProfileSetupHelper = require("../../../../lib/commands/configure/aws-profile-setup-helper");
const messages = require("../../../../lib/commands/configure/messages");
const helper = require("../../../../lib/commands/configure/helper");
  ...
  describe("# test initiateAskProfileSetup", () => {
    ...
    it("| setupAskToken returns valid token and setupVendorId throws error ", (done) => {
      ...
      helper.initiateAskProfileSetup(TEST_CONFIG, (error, askProfile) => {
        ...
        expect(infoStub.args[0][0]).eq(
          `ASK Profile "${TEST_PROFILE}" was successfully created. The details are recorded in ask-cli config file (.ask/cli_config) located at your **HOME** folder.`,
        );
        expect(error).eq(TEST_ERROR_MESSAGE);
        expect(askProfile).eq(undefined);
        done();
    ...
    it("| successfully initiated askProfile and awsProfile", (done) => {
      ...
      helper.initiateAskProfileSetup(TEST_CONFIG, (error, askProfile) => {
        ...
        expect(infoStub.args[3][0]).eq(
          `AWS profile "${TEST_AWS_PROFILE}" was successfully associated with your ASK profile "${TEST_PROFILE}".\n`,
        );
        expect(setAwsProfileStub.args[0][0]).eq(TEST_PROFILE);
