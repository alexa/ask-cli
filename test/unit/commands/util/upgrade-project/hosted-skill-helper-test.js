...
const GitClient = require("../../../../../lib/clients/git-client");
const hostedSkillHelper = require("../../../../../lib/commands/util/upgrade-project/hosted-skill-helper");
const HostedSkillController = require("../../../../../lib/controllers/hosted-skill-controller");
...
const AskResources = require("../../../../../lib/model/resources-config/ask-resources");
const AskStates = require("../../../../../lib/model/resources-config/ask-states");
...
describe("Commands upgrade-project test - hosted skill helper test", () => {
  ...
  const FIXTURE_STATES_CONFIG_FILE_PATH = path.join(
    process.cwd(),
    "test",
    "unit",
    "fixture",
  ...
  describe("# test helper method - checkIfDevBranchClean", () => {
    ...
    it("| dev branch short status is not clean, expect error thrown", () => {
      ...
      // call & verify
      expect(() => hostedSkillHelper.checkIfDevBranchClean(gitClient)).throw(
        CliError,
        `Commit the following files in the dev branch before upgrading project:\n${TEST_OUTPUT}`,
      );
    });
    ...
    it("| origin branch commits difference is not zero , expect error thrown", () => {
      ...
      // call & verify
      expect(() => hostedSkillHelper.checkIfDevBranchClean(gitClient)).throw(
        "Upgrade project failed. Your branch is ahead of origin/dev by 1 commit(s), " +
    ...
    it("| master branch commits difference is not zero , expect error thrown", () => {
      ...
      // call & verify
      expect(() => hostedSkillHelper.checkIfDevBranchClean(gitClient)).throw(
        "Upgrade project failed. Your branch is ahead of master by 1 commit(s), " +
          "Please follow the project upgrade instruction from " +
          "https://github.com/alexa/ask-cli/blob/develop/docs/Upgrade-Project-From-V1.md#upgrade-steps " +
          "to clean your working branch before upgrading project.",
      );
  ...
  describe("# test helper method - createV2ProjectSkeletonAndLoadModel", () => {
    beforeEach(() => {
      ...
      path.join
        .withArgs(TEST_ROOT_PATH, CONSTANTS.FILE_PATH.HIDDEN_ASK_FOLDER, CONSTANTS.FILE_PATH.ASK_STATES_JSON_CONFIG)
  ...
  describe("# test helper method - postUpgradeGitSetup", () => {
    ...
    it("| set Git Credential Helper fails, expect error thrown", (done) => {
      ...
      // call
      hostedSkillHelper.postUpgradeGitSetup(TEST_PROFILE, TEST_DO_DEBUG, gitClient, TEST_REPO_URL, TEST_SKILL_ID, (err) => {
        // verify
