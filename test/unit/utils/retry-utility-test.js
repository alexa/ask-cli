...
const retryUtils = require("../../../lib/utils/retry-utility");
  ...
  describe("# test function retry", () => {
    ...
    [
      ...
      },
      {
        ...
        expectedResult: {
          error:
            "[Error]: Invalid retry configuration: factor. Factor is a required " +
      ...
      },
      {
        ...
        expectedResult: {
          error:
            "[Error]: Invalid retry configuration: factor. " +
      ...
      },
      {
        ...
        expectedResult: {
          error:
            "[Error]: Invalid retry configuration: maxRetry. " +
      ...
      {
        testCase:
          "Valid retry configurations. 3 requests are made. Service returns " +
        ...
        testResponse: {
          defaultResponse: PROGRESS_RESPONSE,
          responseMap: [PROGRESS_RESPONSE, PROGRESS_RESPONSE, SUCCESS_RESPONSE],
        },
        expectedResult: {
          error: null,
    ...
    it("| retry doesn't exceed max interval limit",async ()=>{
      let retryConfig = {
        base: 2000,
        factor: 500,
        maxRetry: 3,
      ...
      retryUtils.retry(retryConfig, retryCall, shouldRetryCondition, (err, res) => {
        expect(spy.lastCall.args[1]).to.be.eq(CONSTANTS.CONFIGURATION.RETRY.MAX_RETRY_INTERVAL)
        done();
      });
      clock.tick(CONSTANTS.CONFIGURATION.RETRY.MAX_RETRY_INTERVAL);
    });
  });
});
