...
const DialogReplayFile = require("../../../lib/model/dialog-replay-file");
    ...
    describe("# test read file content", () => {
      ...
      it("| throws error if input file path does not exist", () => {
        try {
          ...
          // verify
          expect(error).equal(
            `Failed to parse file: ${NOT_EXISTING_DIALOG_REPLAY_FILE_PATH}.` + `\nFile ${NOT_EXISTING_DIALOG_REPLAY_FILE_PATH} not exists.`,
          );
        }
      });
    ...
    describe("# test write file content", () => {
      ...
      it("| throws error if input file path does not exist", () => {
        ...
        try {
          ...
          // verify
          expect(error).equal(
            `Failed to write to file ${NOT_EXISTING_DIALOG_REPLAY_FILE_PATH}.` +
              `\nFile ${NOT_EXISTING_DIALOG_REPLAY_FILE_PATH} not exists.`,
          );
        }
      });
