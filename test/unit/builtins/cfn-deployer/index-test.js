...
const Deployer = require("../../../../lib/builtins/deploy-delegates/cfn-deployer/index");
  ...
  describe("bootstrap", () => {
    ...
    it("should bootstrap deployer", (done) => {
      Deployer.bootstrap(bootstrapOptions, (err, result) => {
        const expected = {
          userConfig: {runtime, handler, templatePath: `./infrastructure/${bootstrapOptions.workspacePath}/skill-stack.yaml`, awsRegion},
        };
        expect(err).eql(null);
  ...
  describe("deploy", () => {
    ...
    beforeEach(() => {
      expectedOutput = {
        ...
        isCodeDeployed: true,
        deployState: {stackId, outputs: [], s3: {bucket: bucketName, key: bucketKey}},
      ...
      expectedErrorOutput = {
        ...
        deployState: {stackId, s3: {bucket: bucketName, key: bucketKey}},
    ...
    it("should throw error when unsupported alexa region is used", (done) => {
      ...
      Deployer.invoke({}, deployOptions, (err, result) => {
        ...
        expectedErrorOutput.resultMessage =
          `The CloudFormation deploy failed for Alexa region "${unsupportedRegion}": ` +
