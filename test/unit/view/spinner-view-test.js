...
describe("View test - spinner view test", () => {
  ...
  describe("# inspect correctness for constructor", () => {
    ...
    it("| make sure SpinnerView class init with default config", () => {
      ...
      const ProxySpinnerView = proxyquire("../../../lib/view/spinner-view", {
        ora: oraStub,
      });
      // call
  ...
  describe("# test class methods", () => {
    ...
    beforeEach(() => {
      ...
      ProxySpinnerView = proxyquire("../../../lib/view/spinner-view", {
        ora: () => ({
          start: startStub,
          stop: stopStub,
          succeed: succeedStub,
    ...
    it('| test SpinnerView class method - restart with default style', () => {
      // call
      const spinner = new ProxySpinnerView();
      spinner.start(TEST_MESSAGE);
      spinner.restart();
      ...
      expect(stopStub.callCount).equal(1);
      expect(stopStub.args[0][0]).equal(undefined);
      expect(startStub.callCount).equal(2);
      expect(startStub.args[0][0]).equal(TEST_MESSAGE);
      expect(startStub.args[1][0]).equal(TEST_MESSAGE);
    });

    it('| test SpinnerView class method - restart with FAIL style', () => {
        // call
        const spinner = new ProxySpinnerView();
        spinner.start(TEST_MESSAGE);
        spinner.restart(SpinnerView.TERMINATE_STYLE.FAIL);
        // expect
        expect(failStub.callCount).equal(1);
        expect(failStub.args[0][0]).equal(undefined);
        expect(startStub.callCount).equal(2);
    ...
    it('| test SpinnerView class method - restart with SUCCEED style and a custom message', () => {
        // call
        const spinner = new ProxySpinnerView();
        const message = 'F00';
        spinner.start(TEST_MESSAGE);
        spinner.restart(SpinnerView.TERMINATE_STYLE.SUCCEED, message);
        // expect
        expect(succeedStub.callCount).equal(1);
        expect(succeedStub.args[0][0]).equal(message);
        expect(startStub.callCount).equal(2);
        expect(startStub.args[0][0]).equal(TEST_MESSAGE);
        expect(startStub.args[1][0]).equal(TEST_MESSAGE);
    });
