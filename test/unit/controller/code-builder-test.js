...
const ZipOnlyBuildFlow = require("../../../lib/builtins/build-flows/zip-only");
const NodeJsNpmBuildFlow = require("../../../lib/builtins/build-flows/nodejs-npm");
...
describe("Controller test - CodeBuilder test", () => {
  ...
  describe("# inspect correctness of execute method", () => {
    ...
    it("| should execute node js npm build flow", (done) => {
      sinon.stub(path, "join").returns("some-file").withArgs(sinon.match.any, sinon.match("package.json")).returns("package.json");
      sinon.stub(fs, "existsSync").returns(false).withArgs(sinon.match("package.json")).returns(true);
