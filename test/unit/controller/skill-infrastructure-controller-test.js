...
const DeployDelegate = require("../../../lib/controllers/skill-infrastructure-controller/deploy-delegate");
...
describe("Controller test - skill infrastructure controller test", () => {
  ...
  describe("# test class method: deployInfraToAllRegions", () => {
    ...
    it("| skill name failed to parse, expect error called back", (done) => {
      ...
      skillInfraController.deployInfraToAllRegions(TEST_DD, (err, res) => {
        ...
        expect(res).equal(undefined);
        expect(err).equal(
          "[Error]: Failed to parse the skill name used to decide the CloudFormation stack name. " +
  ...
  describe("# test class method: _getAlexaDeployRegions", () => {
    ...
    it(
      "| alexaRegion is not default, userConfig regionalOverrides awsRegion is NOT set, " +
        "expect awsRegion is set based on Alexa and AWS region map.",
      (done) => {
        // setup
        const TEST_ALEXA_REGION_EU = "EU";
