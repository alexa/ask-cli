...
describe("Clients test - cli git client", () => {
  ...
  describe("# test configureCredentialHelper", () => {
    ...
    it("| test git configureCredentialHelper execute commands correctly ", () => {
      ...
      sinon.stub(gitClient, "_execChildProcessSync");
      // call
      gitClient.configureCredentialHelper(TEST_CREDENTIAL_SCRIPT_EXECUTION, TEST_GIT_HOST_URL);
      // verify
    ...
    it("| test git configureCredentialHelper fails", () => {
      ...
      sinon.stub(gitClient, "_execChildProcessSync").throws(new CLiError(`${TEST_ERROR}`));
      // call & verify
      expect(() => gitClient.configureCredentialHelper(TEST_CREDENTIAL_SCRIPT_EXECUTION, TEST_GIT_HOST_URL)).throw(
        CLiError,
        `CliError: ${TEST_ERROR}`,
      );
    });
  ...
  describe("# git clone", () => {
    ...
    const TEST_BRANCH = "ASK-CLI";
    const TEST_COMMAND = [
      `git clone --branch ${TEST_BRANCH} ${TEST_CLONE_URL} "${TEST_FOLDER_NAME}" ` +
  ...
  describe("# test _execChildProcessSync", () => {
    ...
    it("| test _execChildProcessSync succeeds ", () => {
      ...
      const proxyHelper = proxyquire("../../../lib/clients/git-client", {
    ...
    it("| test _execChildProcessSync fails ", () => {
      ...
      // call & verify
      expect(() => proxyHelper.prototype._execChildProcessSync(TEST_COMMAND, TEST_VERBOSITY_OPTIONS)).throw(Error, `${TEST_ERROR}`);
    });
  });
});
