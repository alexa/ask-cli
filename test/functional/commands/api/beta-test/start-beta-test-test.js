...
describe('Functional test - ask api start-beta-test', () => {
    const operation = 'start-beta-test';
    ...
    const START_BETA_TEST_COMMAND = `ask api ${operation}`;
    const START_BETA_TEST_SUCCESS_MESSAGE = 'Beta test started successfully';
    const EMPTY_BODY = {};
    ...
    function getStartBetaTestRequestOptions() {
        return {
            url: `${CONSTANTS.SMAPI.ENDPOINT}/v1/skills/${TEST_SKILL_ID}/betaTest/start`,
    ...
    it('| print error if skill-id is not provided', async () => {
        const cmd = START_BETA_TEST_COMMAND;
        const envVar = {};
    ...
    it('| print error when input is an invalid profile', async () => {
        const cmd = `${START_BETA_TEST_COMMAND} -s ${TEST_SKILL_ID} -p ${TEST_INVALID_PROFILE}`;
        const envVar = {};
        const httpMockConfig = [];
        const expectationHandler = (msgCatcher) => {
            expect(msgCatcher.info).equal('');
    ...
    it('| can get correct http response, when valid skill Id is provided', async () => {
        const inputOptions = getStartBetaTestRequestOptions();
        const cmd = `${START_BETA_TEST_COMMAND} -s ${TEST_SKILL_ID}`;
        const envVar = {};
        const httpMockConfig = [{
        ...
        const expectationHandler = (msgCatcher) => {
            expect(msgCatcher.info).equal(START_BETA_TEST_SUCCESS_MESSAGE);
            expect(msgCatcher.warn).equal('');
