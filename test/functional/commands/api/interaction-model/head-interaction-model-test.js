...
describe('Functional test - ask api head-interaction-model', () => {
    const operation = 'head-interaction-model';
    ...
    it('| print error when skill-id is not provided', async () => {
        const cmd = 'ask api head-interaction-model';
        const envVar = {};
    ...
    it('| print error when locale is not provided ', async () => {
        const cmd = `ask api head-interaction-model -s ${TEST_SKILL_ID}`;
        const envVar = {};
        const httpMockConfig = [];
        const expectationHandler = (msgCatcher) => {
            expect(msgCatcher.info).equal('');
    ...
    it('| print error when input invalid profile', async () => {
        const cmd = `ask api head-interaction-model -s ${TEST_SKILL_ID} -l ${TEST_LOCALE} -p ${TEST_INVALID_PROFILE}`;
        const envVar = {};
        const httpMockConfig = [];
        const expectationHandler = (msgCatcher) => {
            expect(msgCatcher.info).equal('');
    ...
    it('| print error when input stage is invalid', async () => {
        const cmd = `ask api head-interaction-model -s ${TEST_SKILL_ID} -l ${TEST_LOCALE} -g ${TEST_INVALID_STAGE}`;
        const envVar = {};
        const httpMockConfig = [];
        const expectationHandler = (msgCatcher) => {
            expect(msgCatcher.info).equal('');
    ...
    it('| print error when input locale is invalid', async () => {
        const cmd = `ask api head-interaction-model -s ${TEST_SKILL_ID} -l invalidLocale`;
        const envVar = {};
        const httpMockConfig = [];
        const expectationHandler = (msgCatcher) => {
            expect(msgCatcher.info).equal('');
    ...
    it('| can get correct http response when profile is set to default ', async () => {
        const cmd = `ask api head-interaction-model -s ${TEST_SKILL_ID} -l ${TEST_LOCALE}`;
        const envVar = {};
        const httpMockConfig = [{
    ...
    it('| can get correct http response when profile is set by valid input', async () => {
        const cmd = `ask api head-interaction-model -s ${TEST_SKILL_ID} -l ${TEST_LOCALE} -p ${TEST_VALID_PROFILE}`;
        const envVar = {};
        const httpMockConfig = [{
