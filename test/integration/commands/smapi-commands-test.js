...
const skillManifest = require("../../integration/fixtures/skill-manifest.json");
const catalogUploadBody = require("../../integration/fixtures/catalog-upload.json");
const inSkillProductRequestBody = require("../../integration/fixtures/create-in-skill-product-request.json");
const accountLinkingRequest = require("../../integration/fixtures/account-linking-request.json");
const interactionModel = require("../../integration/fixtures/interaction-model.json");
...
const addCoveredCommand = (args) => {
  const smapiCmd = args[1];
  if (testedCommands.has(smapiCmd)) {
    console.warn(`${smapiCmd} already has been covered!`);
  }
  testedCommands.add(smapiCmd);
...
parallel('smapi command test', () => {
  ...
  it("| should update account linking", async () => {
    const args = [
      ...
      JSON.stringify(accountLinkingRequest),
    ];
    addCoveredCommand(args);
    const result = await run(askCmd, args, {...options, parse: false});
    expect(result).include("Command executed successfully!");
  ...
  it("| should generate credentials for alexa hosted skill", async () => {
    const args = [
      ...
      skillId,
      "--repository-url",
      "https://git-codecommit.us-east-1.amazonaws.com/v1/repos/e9ac9d7f-5c4f-4c3b-8e41-1b347f625d95",
      "--repository-type",
      "GIT",
    ];
    addCoveredCommand(args);
  ...
  it("| should update annotations for nlu annotation sets", async () => {
    const args = [
      ...
      "--content-type",
      contentType,
      "--update-nlu-annotation-set-annotations-request",
  ...
  it("| should create job definition for IM", async () => {
    const args = [subCmd, "create-job-definition-for-interaction-model", "--job-definition", interactionModelJobDefinition];
    addCoveredCommand(args);
    const result = await run(askCmd, args, options);
    expect(result).be.an("object");
  });
