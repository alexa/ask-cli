...
class ListrReactiveTask {
   ...
   * @param {Function} taskHandle the task handle which will be executed later, in the interface of:
...
class MultiTasksView {
  ...
  start(callback) {
    ...
    this.taskRunner
      ...
      .catch((listrError) => {
        let errorMessage = listrError.resultMessage || listrError.message || listrError;
        if (listrError.errors) {
          errorMessage = listrError.errors.map((e) => e.resultMessage || e.message || e).join("\n");
        }
        callback({
