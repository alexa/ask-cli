...
class Messenger {
    ...
    constructor({ doDebug }) {
        ...
        this._buffer = [];
        this.flushDebugMessages = this.flushDebugMessages.bind(this);
        // This flag is for debug use in order to
        // determine whether commands is working on debug mode
        this.setDoDebug(doDebug);
        return instance;
    }

    setDoDebug(doDebug = false) {
        this.doDebug = doDebug;
        if (this.doDebug) {
            if (!process.listeners('exit').includes(this.flushDebugMessages)) {
                process.on('exit', this.flushDebugMessages);
            }
        }
    }
    ...
    flushDebugMessages() {
        this._buffer.forEach(item => {
            if (item.operation === 'DEBUG' && item.msg) {
    ...
    dispose() {
        ...
        instance = null;
        this._buffer = [];
        process.removeListener('exit', this.flushDebugMessages);
    }
