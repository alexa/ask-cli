...
const DialogOutputFile = require('@src/model/dialog-output-file');
...
module.exports = class SkillSimulationController {
    ...
    constructor(configuration) {
        ...
        const { skillId, locale, stage, profile, output, debug, smapiClient } = configuration;
        ...
        this.locale = locale;
        this.stage = stage;
        this.dialogOutputInstance = new DialogOutputFile(output);
    }
    ...
    startSkillSimulation(utterance, newSession, callback) {
        this.dialogOutputInstance.startInvocation({ utterance, newSession });
    ...
    getSkillSimulationResult(simulationId, callback) {
        ...
        Retry.retry(retryConfig, retryCall, shouldRetryCondition, (err, res) => {
            ...
            this.dialogOutputInstance.endInvocation({ body: res.body });
            if (!res.body.status) {
