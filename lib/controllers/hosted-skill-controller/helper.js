...
function pollingSkillStatus(smapiClient, skillId, callback) {
  ...
  const RESOURCE_LIST = [
    CONSTANTS.SKILL.RESOURCES.MANIFEST,
...
function handleSkillStatus(response, skillId, callback) {
  ...
  if (
    response.hostedSkillProvisioning === CONSTANTS.HOSTED_SKILL.PROVISIONING_STATUS.SUCCESS &&
    ...
    response.hostedSkillProvisioning === CONSTANTS.HOSTED_SKILL.PROVISIONING_STATUS.IN_PROGRESS ||
...
function downloadAuthInfoScript(callback) {
  ...
  const authInfoPath = path.join(os.homedir(), CONSTANTS.FILE_PATH.ASK.HIDDEN_FOLDER, CONSTANTS.FILE_PATH.ASK.AUTH_INFO);
...
function downloadAskPrePushScript(callback) {
  ...
  const scriptFolderPath = path.join(os.homedir(), CONSTANTS.FILE_PATH.ASK.HIDDEN_FOLDER, CONSTANTS.FILE_PATH.ASK.SCRIPTS_FOLDER.NAME);
