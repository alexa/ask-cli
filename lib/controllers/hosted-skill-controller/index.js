...
module.exports = class HostedSkillController {
    ...
    clone(skillId, skillName, projectPath, callback) {
        ...
        this.getHostedSkillMetadata(skillId, (metadataErr, metadata) => {
            ...
            // 2. download credential helper
            fs.mkdirSync(path.join(projectPath, CONSTANTS.FILE_PATH.HIDDEN_ASK_FOLDER, 'scripts'));
            const filePath = path.join(projectPath, CONSTANTS.FILE_PATH.HIDDEN_ASK_FOLDER, 'scripts', 'git-credential-helper');
            const templateUrl = CONSTANTS.HOSTED_SKILL.GIT_HOOKS_TEMPLATES.CREDENTIAL_HELPER.URL;
            this.downloadGitHelperFile(templateUrl, filePath, (credHelperError) => {
                if (credHelperError) {
                    return callback(credHelperError);
                }
                // 3. clone project, set up git
                cloneFlow.cloneProjectFromGit(projectPath, skillName, this.profile, metadata.repository.url, this.doDebug);
                // 4. check skill-package content
    ...
    /**
     * To download git helper file
     * @param {String} templateUrl The template url
     * @param {String} filePath The file path
     * @param {callback} callback { error, response }
     */
    downloadGitHelperFile(templateUrl, filePath, callback) {
        const params = {
            url: templateUrl,
