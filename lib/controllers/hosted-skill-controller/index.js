...
module.exports = class HostedSkillController {
    ...
    /**
     * To download the pre-push hook to the project git folder and check other S3 scripts
     * @param {String} folderName the project folder name
     * @param {callback} callback { error, response }
     */
    downloadGitHookScript(folderName, callback) {
        this.checkS3Scripts((err) => {
            if (err) {
                return callback(err);
            }
            const prePushUrl = CONSTANTS.HOSTED_SKILL.S3_SCRIPTS.PRE_PUSH.URL;
    ...
    /**
     * To check all scripts from S3 and update local scripts if remote scripts are changed
     * @param {callback} callback { error, response }
     */
    checkS3Scripts(callback) {
        this.checkAuthInfoScript((authInfoErr) => {
            if (authInfoErr) {
                return callback(authInfoErr);
            }
            this.checkAskPrePushScript((prePushErr) => callback(prePushErr || null));
        });
    }

    /**
     * To check the auth_info script and update the local one if the remote one are changed
     * @param {callback} callback { error, response }
     */
    checkAuthInfoScript(callback) {
        const authInfoUrl = CONSTANTS.HOSTED_SKILL.S3_SCRIPTS.AUTH_INFO.URL;
        const authInfoPath = path.join(os.homedir(),
            CONSTANTS.FILE_PATH.ASK.HIDDEN_FOLDER,
            CONSTANTS.FILE_PATH.ASK.AUTH_INFO);
        helper.checkScript(authInfoUrl, authInfoPath, (err) => callback(err || null));
    }

    /**
     * To check the ask-pre-push script and update the local one if the remote one are changed
     * @param {callback} callback { error, response }
     */
    checkAskPrePushScript(callback) {
        const askScriptUrl = CONSTANTS.HOSTED_SKILL.S3_SCRIPTS.ASK_PRE_PUSH.URL;
        ...
        helper.checkScript(askScriptUrl, askFilePath, (err) => callback(err || null));
    }
};
