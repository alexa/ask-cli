...
module.exports = class SkillMetadataController {
    ...
    deploySkillPackage(vendorId, ignoreHash, callback) {
        ...
        // 1.1 check if it's a conversations skill, prepare the upload package inside a tmp directory
        const tmpSkillPackageSrc = this.synthesize(skillPackageSrc);

        // 2.compare hashcode between current and previous status to decide if necessary to upload
        hashUtils.getHash(tmpSkillPackageSrc, (hashErr, currentHash) => {
            if (hashErr) {
            ...
            this.putSkillPackage(tmpSkillPackageSrc, skillId, skillId ? null : vendorId, (putErr, currentSkillId) => {
                if (putErr) {
    ...
    /**
     * Synthesizes the authored skill package into a deployable SMAPI skill package.
     * Currently, it generates a deployable skill package only for the Alexa Conversations skills
     * that have ./conversations directory in the skill-package.
     * 
     * @param skillPackageSrc - Authored skill package root.
     * @returns - location of the deployable skill package.
     */
    synthesize(skillPackageSrc) {
        const compilerConfig = CONSTANTS.COMPILER;
        const synthConfig = CONSTANTS.SYNTHESIZE;

        /**
         * If authored skill package does not contains converstations directory.
         */
        if(!fs.existsSync(path.join(skillPackageSrc, compilerConfig.SKILL_PACKAGE.CONVERSATIONS))) {
            return skillPackageSrc;
        }

        /**
         * If authored skill package does not contains build/ directory.
         * It means askx compile didn't happen.
         */
        if(!fs.existsSync(path.join(skillPackageSrc, compilerConfig.SKILL_PACKAGE.OUTDIR))) {
            Messenger.getInstance().error(`Skills containing ${compilerConfig.SKILL_PACKAGE.CONVERSATIONS} directory must be compiled before deployment.`);
            process.exit() // TODO Fix me. Wire the callbacks.
        }

        /**
         * Else, it's a compiled Alexa Conversations skill.
         */
        const targetSkillPkgDir = path.join(CONSTANTS.FILE_PATH.ASK.HIDDEN_FOLDER, synthConfig.TARGETDIR);
        /**
         * 1. Clean up the old synthesized artifacts.
         */
        fs.rmdirSync(targetSkillPkgDir, { recursive: true });
        /**
         * 2. Copy the authored skill-package to the target location.
         */
        fs.copySync(skillPackageSrc, targetSkillPkgDir);
        /*
            * 3. Remove the conversations/*.acdl as they are not deployed to SMAPI
            */
        fs.rmdirSync(path.join(targetSkillPkgDir, compilerConfig.SKILL_PACKAGE.CONVERSATIONS), { recursive: true });
        /**
         * 4. Rename the build/*.json (IR) to conversations/*.json as they are deployed to SMAPI
         */
        fs.renameSync(
            path.join(targetSkillPkgDir, compilerConfig.SKILL_PACKAGE.OUTDIR),
            path.join(targetSkillPkgDir, synthConfig.CONVERSATIONS));
        /**
         * 5. Return the target path so deployer can use this path 
         */
        return targetSkillPkgDir;
    }
