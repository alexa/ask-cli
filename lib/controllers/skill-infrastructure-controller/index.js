    ...
    updateSkillManifestWithDeployResult(rawDeployResult, callback) {
        ...
        hashUtils.getHash(ResourcesConfig.getInstance().getSkillMetaSrc(this.profile), (hashErr, currentHash) => {
            ...
            this._ensureSkillManifestGotUpdated((manifestUpdateErr) => {
                ...
                const skillManifest = ResourcesConfig.getInstance().getSkillMetaManifest(this.profile);
                Messenger.getInstance().info(`  The api endpoints of ${skillManifest} `
                + 'have been updated from the skill infrastructure deploy results.');
                callback();
            });
        });
    }
