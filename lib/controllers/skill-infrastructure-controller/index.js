...
module.exports = class SkillInfrastructureController {
  ...
  deployInfraToAllRegions(dd, callback) {
    const skillName =
      stringUtils.filterNonAlphanumeric(Manifest.getInstance().getSkillName()) ||
    ...
    multiTasksView.start((taskErr, taskResult) => {
      ...
      if (result) {
        R.keys(result)
          .filter((alexaRegion) => result[alexaRegion].isDeploySkipped)
          .forEach((alexaRegion) => {
  ...
  _getAlexaDeployRegions(regionsList, userConfig) {
    ...
    regionsList.forEach((alexaRegion) => {
      const awsRegion =
        alexaRegion === "default" ? userConfig.awsRegion : R.path(["regionalOverrides", alexaRegion, "awsRegion"], userConfig);
