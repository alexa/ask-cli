...
module.exports = class AuthorizationController {
    ...
    async getAccessToken(profile) {
        if (profile === CONSTANTS.PLACEHOLDER.ENVIRONMENT_VAR.PROFILE_NAME && process.env.ASK_ACCESS_TOKEN) {
            return process.env.ASK_ACCESS_TOKEN;
        }
        const tokenObj = AppConfig.getInstance().getToken(profile);
        if (this.oauthClient.isValidToken(tokenObj)) {
            return tokenObj.access_token;
        }
        const refreshedTokenObj = await this.oauthClient.getAccessToken(tokenObj);
        if (profile !== CONSTANTS.PLACEHOLDER.ENVIRONMENT_VAR.PROFILE_NAME) {
            AppConfig.getInstance().setToken(profile, refreshedTokenObj);
            AppConfig.getInstance().write();
        }
        return refreshedTokenObj.access_token;
    }
