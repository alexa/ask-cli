...
module.exports = class LWAAuthCodeClient {
    ...
    getAccessToken(token) {
        const url = new URL(this.config.tokenPath, this.config.tokenHost);
        const client = new HttpClient({ debug: this.config.doDebug });
        const body = `grant_type=refresh_token&client_secret=${this.config.clientConfirmation}`
        + `&client_id=${this.config.clientId}&refresh_token=${token.refresh_token}`;
        const options = {
            url: `${url}`,
            method: 'POST',
            body,
        ...
        return client.invoke(options)
            .then(res => {
                ...
                const expiresIn = resBody.expires_in;
                const tokenBody = R.clone(resBody);
                tokenBody.expires_at = this._getExpiresAt(expiresIn).toISOString();
                return tokenBody;
            }).catch((err) => {
                throw new Error(`Refresh LWA tokens failed, please run "ask configure" to manually update your tokens. Inner Error: ${err.message}`);
            });
    }
     ...
     */
    isValidToken(token) {
        return token.expires_at && !isAfter(new Date(), parseISO(token.expires_at));
    }
