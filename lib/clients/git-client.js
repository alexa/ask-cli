...
module.exports = class GitClient {
    ...
    clone(cloneUrl, branch, folderName) {
        // allow non ".git" clones for private repositories
        // as long as git is authenticated correctly, it will work
        var commands = "";
        if(!cloneUrl.endsWith(".git")) {
            console.log("Executing git custom command..");
            commands = [`git clone ${cloneUrl} ${folderName} ${this.verbosityOptions.showOutput === false ? ' --quiet' : ''}`];
        } else {
            commands = [`git clone --branch ${branch} ${cloneUrl} ${folderName} ${this.verbosityOptions.showOutput === false ? ' --quiet' : ''}`];
        }
