...
function request(options, operation, doDebug, callback) {
    ...
    requestLib(requestOptions, (error, response) => {
        ...
        if (error || !response) {
            return callback(`[Error]: Failed to make request to ${operation}. Please make sure "${requestOptions.url}" is responding.`);
        }
...
function resolveUserAgent() {
    const cliUserAgentStr = `ask-cli-x/${pkg.version} Node/${process.version} ${os.type()}/${os.release()}`;
    if (stringUtils.isNonBlankString(process.env.ASK_DOWNSTREAM_CLIENT)) {
        return `${process.env.ASK_DOWNSTREAM_CLIENT} (ask-cli-x downstream client) ${cliUserAgentStr}`;
    }
