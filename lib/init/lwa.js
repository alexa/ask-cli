...
// Public
module.exports.login = (needBrowser, callback) => {
    const SCOPES = 'alexa::ask:skills:readwrite alexa::ask:models:readwrite alexa::ask:skills:test';
    ...
    if (needBrowser) {
        ...
        passAuthcode(REDIRECT_URI, OAuth, callback);
    }
};
...
function passAuthcode(REDIRECT_URI, OAuth, callback){
    let question = [
        {
            type: 'input',
            name: 'authCode',
    ...
    inquirer.prompt(question).then(
        (answer) => {
            getTokens(REDIRECT_URI, answer.authCode, OAuth, callback);
        }
    );
}
