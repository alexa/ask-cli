const {kebabCase, standardize} = require("../../utils/string-utils");
const CliError = require("../../exceptions/cli-error");
const {customizationMap} = require("./customizations/parameters-map");
...
class CliCustomizationProcessor {
  ...
  _appendSecondLevelProperty(customizationMetadata, parentName, rootName, secondLevelDefinition, parentRequired, definitions) {
    ...
    Object.keys(secondLevelDefinition.properties || []).forEach((key) => {
      ...
      if (property.$ref) {
        ...
        if (!schema.enum) {
          throw new CliError(
            "Cannot unwrap more then 2 levels deep. Please use customParameters or " +
  ...
  _ensureNumberOfProperties(parameterName, properties) {
    if (properties && Object.keys(properties).length >= MAX_NESTED_PROPERTIES) {
      throw new CliError(
        `${parameterName} - number of body properties ` + `exceeds ${MAX_NESTED_PROPERTIES}. Please use customParameters.`,
      );
    }
  }
}
