...
const {apiToCommanderMap, customizationMap} = require("./customizations/parameters-map");
...
const { SmapiApiClient } = require("../../clients/smapi-client/smapiApiClient");
const unflatten = require("../../utils/unflatten");
const {getParamNames, standardize, canParseAsJson, kebabCase} = require("../../utils/string-utils");
...
const _sdkFunctionName = (swaggerApiOperationName) =>
  `call${swaggerApiOperationName.charAt(0).toUpperCase() + swaggerApiOperationName.slice(1)}`;
...
const _showCheckStatusHint = (locationHeader, profile) => {
  const [baseUrl, queryStringPart] = locationHeader.split("?");
  ...
  if (swaggerMatch && modelIntrospector.modelJson.paths[swaggerMatch.swaggerPatternUrl].get) {
    ...
    const cliParams = Object.keys(parameters)
      .map((key) => {
    ...
    Messenger.getInstance().warn(
      `This is an asynchronous operation. Check the progress using the following command: ${statusCheckHintCommand}`,
    );
  } else {
    Messenger.getInstance().warn(`This is an asynchronous operation. Check the progress using the following url: ${locationHeader}`);
  }
};
 ...
 */
const smapiCommandHandler = async (swaggerApiOperationName, flatParamsMap, commanderToApiCustomizationMap, inputCmdObj, modelIntrospector) => {
  new AppConfig();
  ...
  const smapiApiClient = new SmapiApiClient(inputOpts.debug, profile);
  const accessTokenConfig = {
    clientId: authorizationController.oauthClient.config.clientId,
    clientSecret: authorizationController.oauthClient.config.clientConfirmation,
    accessToken: await getAuthToken(authorizationController, profile),
  }
  ...
  return client[functionName](...args).then((response) => {
    const {body, headers, statusCode} = response;
    let result = "";
    if (inputOpts.debug) {
...
const getAuthToken = async (authorizationController, profile) => {
  return new Promise((resolve, reject) => {
    authorizationController.tokenRefreshAndRead(profile, (tokenErr, authToken) => {
      (tokenErr)? reject(tokenErr) : resolve(authToken);
    });
  });
};
