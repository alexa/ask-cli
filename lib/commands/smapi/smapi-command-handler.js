...
const smapiCommandHandler = async (swaggerApiOperationName, flatParamsMap, commanderToApiCustomizationMap, inputCmdObj, modelIntrospector) => {
    ...
    const { debug } = inputOpts;

    new AppConfig();
    const authorizationController = new AuthorizationController({ auth_client_type: 'LWA', doDebug: debug });
    const accessToken = await authorizationController.getAccessToken(profile);

    const client = makeSmapiClient({ profile, accessToken, debug });
