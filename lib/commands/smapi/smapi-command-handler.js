...
/**
 * Matches url against swagger url patter. Returns the url pattern and matched parameters if matched.
 * @param {string} url url to match
 * @param {string} swaggerPatternUrl  swagger patter url
 */
const matchSwaggerPattern = (url, swaggerPatternUrl) => {
...
/**
 * Makes cli response from smapi response location header
 * @param {Object} locationHeader
 */
const makeResultFromLocationHeader = (locationHeader) => {
    ...
    for (const path of paths) {
        swaggerMatch = matchSwaggerPattern(baseUrl, path);
    ...
    return jsonView.toString({ ...parameters, statusCheckHintCommand, locationHeaderValue: value });
};
...
const parseSmapiResponse = (response) => {
    ...
    if (statusCode === 202 && locationHeader) {
        result = makeResultFromLocationHeader(locationHeader);
