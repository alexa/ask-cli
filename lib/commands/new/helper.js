...
function loadSkillProjectModel(projectPath, profile) {
    ...
    const manifestFile = ResourcesConfig.getInstance().getSkillMetaManifest(profile);
    const manifestPath = path.join(skillPackageSrc, manifestFile);
    if (!fs.existsSync(manifestPath)) {
        throw new Error(`[Error]: Invalid skill project structure. Please make sure ${manifestFile} exists in ${skillPackageSrc}.`);
    }
