...
class PythonRunFlow extends AbstractRunFlow {
  ...
  constructor({skillInvocationInfo, waitForAttach, debugPort, token, skillId, runRegion, watch}) {
    const sitePkgLocationsStr = childprocess
      .execSync('python3 -c "import site; import json; print(json.dumps(site.getsitepackages()))"')
      .toString();
    ...
    super({
      ...
      args: [
        ...
        "--skillFilePath",
        path.join(`${skillInvocationInfo.skillCodeFolderName}`, `${skillInvocationInfo.skillFileName}.py`),
        "--region",
        runRegion,
      ],
      ext: "py,json,txt",
