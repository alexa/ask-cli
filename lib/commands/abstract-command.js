...
class AbstractCommand {
    ...
    _registerAction(commander) {
        ...
        commander.action((...args) => new Promise((resolve) => {
            ...
            const askCliVersion = semver.parse(packageJson.version);
            // don't remind new versions updates if ASK CLI is a prerelease version
            const isPrereleaseVersion = (askCliVersion.prerelease || []).length > 0;

            // Check if a new CLI version is released and prompt the users to update
            this._remindsIfNewVersion(commandInstance.debug, isCredentialHelperCmd || process.env.ASK_SKIP_NEW_VERSION_REMINDER || isPrereleaseVersion, () => {
                try {
