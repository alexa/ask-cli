        ...
        httpClient.request({
            ...
            if (err) {
                ...
                if (packageJson.version !== latestVersion) {
                    const semVers = packageJson.version.split('.').map(x => parseInt(x, 10));
                    const latestSemVers = latestVersion.split('.').map(x => parseInt(x, 10));
                    if (semVers[0] < latestSemVers[0]) {
...
New major version (v${latestVersion}) of ${CONSTANTS.APPLICATION_NAME} is available now. Current version v${packageJson.version}. Please update!\n`);
                    } else if (semVers[0] === latestSemVers[0] && semVers[1] < latestSemVers[1]) {
                        Messenger.getInstance().info(`[Info]: \
