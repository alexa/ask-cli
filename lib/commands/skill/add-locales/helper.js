...
function addLocales(selectedLocales, profile, doDebug, callback) {
  ...
  _getNewLocaleModelUri(selectedLocales, profile, doDebug, (resolveErr, iModelSourceByLocales) => {
    ...
    const iModelFolderPath = path.join(
      process.cwd(),
      ResourcesConfig.getInstance().getSkillMetaSrc(profile),
      CONSTANTS.FILE_PATH.SKILL_PACKAGE.INTERACTION_MODEL,
      "custom",
    );
...
function _retrieveTemplatesByLanguage(templateSet, doDebug, callback) {
  ...
  async.forEach(
    templateSet.values(),
    (templateUrl, loopCallback) => {
      httpClient.request(
        {
          url: templateUrl,
          method: CONSTANTS.HTTP_REQUEST.VERB.GET,
        },
        "GET_INTERACTION_MODEL_TEMPLATE",
        doDebug,
        (error, response) => {
          if (error) {
