...
function _getNewLocaleModelUri(selectedLocales, profile, doDebug, callback) {
    ...
    httpClient.request({
        ...
        if (error || !templateMapResponse.statusCode || templateMapResponse.statusCode !== 200) {
            return callback('Failed to retrieve the template list. Please run again with --debug to check more details.');
        }
        let templateIndexMap = templateMapResponse.body;
        if (typeof templateMapResponse.body === 'string') {
            templateIndexMap = JSON.parse(templateMapResponse.body);
        }
