...
function initiateAskProfileSetup(config, callback) {
    askProfileSetupHelper.setupAskToken(config, (accessTokenError, accessToken) => {
        ...
        Messenger.getInstance().info(`ASK Profile "${config.askProfile}" was successfully created. The details are recorded in ask-cli config file (.ask/cli_config) located at your **HOME** folder.`);
        ...
        askProfileSetupHelper.setupVendorId(config, (vendorIdError, vendorId) => {
            ...
            awsProfileSetupHelper.setupAwsProfile(config, (awsProfileError, awsProfile) => {
                ...
                if (awsProfile) {
                    Messenger.getInstance().info(`AWS profile "${awsProfile}" was successfully associated with your ASK profile "${config.askProfile}".\n`);
                }
