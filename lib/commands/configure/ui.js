...
function getAuthCode(callback) {
  inquirer
    .prompt(questions.REQUEST_AUTH_CODE)
    .then((answer) => {
...
function createNewProfile(callback) {
  inquirer
    .prompt(questions.REQUEST_ASK_PROFILE_NAME)
    .then((answer) => {
...
function requestAwsProfileName(awsProfileList, callback) {
  inquirer
    .prompt(questions.REQUEST_AWS_PROFILE_NAME(awsProfileList))
    .then((answer) => {
...
function confirmSettingAws(callback) {
  inquirer
    .prompt(questions.SET_UP_AWS_PROFILE)
    .then((answer) => {
...
function selectEnvironmentVariables(callback) {
  inquirer
    .prompt(questions.USE_ENVIRONMENT_VARIABLES)
    .then((answer) => {
      callback(null, answer.choice);
    })
    .catch((error) => {
...
function addNewCredentials(callback) {
  ...
  inquirer
    .prompt(questions.REQUEST_ACCESS_SECRET_KEY_AND_ID)
    .then((answer) => {
