...
function deployResources(options, callback) {
    ...
    _ensureSkillExists(options, (err) => {
        if (err) {
            return callback(err);
        }
...
with build flow ${codeProperty.buildFlow}.`);
            ...
            helper.deploySkillInfrastructure(profile, doDebug, ignoreHash, (infraErr) => {
                ...
                _deploySkillMetadata(options, (errMetadataError) => {
                    if (err) {
                        return callback(errMetadataError);
                    }
                    callback();
                });
            });
...
function _ensureSkillExists(options, callback) {
    const { profile, doDebug } = options;
    if (ResourcesConfig.getInstance().getSkillId(profile)) {
        process.nextTick(() => callback());
        return;
    }
    const smapiClient = new SmapiClient({ profile, doDebug });
    let vendorId;
    try {
    ...
    smapiClient.skill.createSkill(manifest, vendorId, (err, res) => {
        if (err) {
            return callback(err);
        }

        ResourcesConfig.getInstance().setSkillId(profile, res.body.skillId);
        ResourcesConfig.getInstance().write();
        callback();
    });
}
