...
class DeployCommand extends AbstractCommand {
    ...
    handle(cmd, cb) {
        ...
        if (cmd.target && !allowedTargets.includes(cmd.target)) {
            return cb(new CliError(`Target ${cmd.target} is not supported. Supported targets: ${allowedTargets}.`));
        }
...
function deployResources(options, callback) {
    const { profile, doDebug, target } = options;
    _deploySkillMetadata(options, (err) => {
        if (err) {
            return callback(err);
        }

        if (target === CONSTANTS.DEPLOY_TARGET.SKILL_METADATA) {
            return callback();
        }
