...
function _pollingStackStatus(reporter, cloudformationClient, stackId, callback) {
    ...
    const terminateCondition = retryResponse => !retryResponse || retryResponse.StackStatus.endsWith('IN_PROGRESS');
        ...
        stackResources.forEach((stackResource) => {
            ...
            if (stackResource.ResourceStatus === 'DELETE_COMPLETE') {
                errorsList.push({
                    ...
                    resourceType: stackResource.ResourceType,
                    resourceStatus: stackResource.ResourceStatus,
                    reason: `Resource deleted. For detailed reasoning, please check the "events" tab in the AWS console for CloudFormation stackId (${stackId}).`
                });
            }
        });
