...
function deployStack(reporter, options, callback) {
    const {
        awsProfile, awsRegion, templatePath, skillId, templateParameters, stackName,
        stackId, lambdaRuntime, lambdaHandler, codeBucket, codeKey, codeVersion
    } = options;
    ...
    _mergeTemplateParameters(stackOptions.stackParameters, templateParameters);
...
function _mergeTemplateParameters(stackParameters, templateParameters = {}) {
    Object.keys(templateParameters).forEach(ParameterKey => {
        const ParameterValue = templateParameters[ParameterKey];
        const index = stackParameters.findIndex(p => p.ParameterKey === ParameterKey);
        if (index > -1) {
            stackParameters[index].ParameterValue = ParameterValue;
        } else {
            stackParameters.push({ ParameterKey, ParameterValue });
        }
    });
}
