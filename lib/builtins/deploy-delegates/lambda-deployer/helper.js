...
function _createLambdaFunction(reporter, lambdaClient, options, callback) {
    ...
    const retryCall = (loopCallback) => {
        ...
        lambdaClient.createLambdaFunction(functionName, iamRoleArn, zipFile, functionConfig, (lambdaErr, lambdaData) => {
            if (lambdaErr) {
...
function _updateLambdaFunction(reporter, lambdaClient, options, callback) {
    const { alexaRegion, zipFilePath, userConfig, deployState } = options;
    const zipFile = fs.readFileSync(zipFilePath);
    const functionArn = deployState.lambda.arn;
    const functionConfig = _getLambdaFunctionConfig(alexaRegion, userConfig);
    let { revisionId } = deployState.lambda;
    lambdaClient.updateFunctionCode(zipFile, functionArn, revisionId, (codeErr, codeData) => {
        if (codeErr) {
            return callback(codeErr);
        }
        reporter.updateStatus(`Update a lambda function (${functionArn}) in progress...`);
        revisionId = codeData.RevisionId;
        lambdaClient.updateFunctionConfiguration(functionArn, functionConfig, revisionId, (configErr, configData) => {
            if (configErr) {
                return callback(null, {
